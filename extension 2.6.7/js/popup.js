let APP, API_DOMAIN, MARKETPLACE_DOMAIN
...
const Login = () => {
    ...
    if (!email) {
        $('#login-email-error').html('Please input email.').show()
        return
    } else if (!password) {
        $('#login-password-error').html('Please input password.').show()
        return
    }
    ToggleLoading(true)
    HttpRequest({
...
const SendOpportunitiesInquiry = () => {
    ...
    if (!opportunities.length) {
        $('#opportunities-error').html('Please select an opportunity.').show()
        return
    }
    ToggleLoading(true)
    $('#opportunities-inquiry-button').attr('disabled', 'disabled')
...
const SubmitFeedback = () => {
    ...
    if (!contact) return Close()
    if (feedback) {
        $('#feedback-error-message').show().find('span').html('Cannot submit feedback for this site.')
        return
    }
    ToggleLoading(true)
...
const UpdatePassword = () => {
    ...
    if (!current_password) {
        $('#account-current-password-error').html('Please input current password.').show()
        return
    } else if (!password) {
        $('#account-password-error').html('Please input password.').show()
        return
    } else if (password != password_confirmation) {
        $('#account-password-error').html('Password confirmation does not match.').show()
        return
    }
    ToggleLoading(true)
    HttpRequest({
...
const RequestPublisher = () => {
    ...
    if (!message) {
        $('#request-publisher-message-error').html('Please input message.').show()
        return;
    }
    ToggleLoading(true)
    HttpRequest({
